...
class CDFM(nn.Module):
    ...
    def __init__(self, feature_sizes, embedding_size=4,
        ...
        init conv part
        """
        self.conv = nn.Conv2d(1,1,(self.kernel,self.kernel))
        self.conv_embeddings = nn.ModuleList(
            [nn.Embedding(feature_size, self.embedding_size) for feature_size in self.feature_sizes])
    ...
    def forward(self, Xi, Xv):
        ...
        conv part
        """
        conv_emb = [(torch.sum(emb(Xi[:, i, :]), 1).t() * Xv[:, i]).t() for i, emb in enumerate(self.conv_embeddings)]
        conv_res = torch.stack(conv_emb)
        conv_res = conv_res.transpose(1,0)
        conv_res = conv_res.unsqueeze(1)
        conv_2d_res = self.conv(conv_res)
        conv_2d_res = conv_2d_res.mean(dim=-1)
        conv_2d_res = conv_2d_res.mean(dim=-1)
        """
            deep part
        """
        ...
        total_sum = torch.sum(fm_first_order, 1) + \
                    torch.sum(fm_second_order, 1) + torch.sum(deep_out, 1) +torch.sum(conv_2d_res, 1) + self.bias
        #total_sum = torch.sum(fm_first_order, 1) + \
    ...
    def fit(self, loader_train, loader_val, optimizer, epochs=100, verbose=False, print_every=100):
        ...
        for _ in range(epochs):
            ...
            # save ckpt
            torch.save(checkpoint,  'model_save/epoch_{}.pkl'.format(_))
